...
class RandomErasing(object):
    ...
    def __init__(self, p=0.5, minScale=0.02,maxScale=0.33, minRatio=0.3,maxRatio=3.3, value=0, inplace=False):
        assert isinstance(value, (numbers.Number, str, tuple, list))
        ratio = (minRatio,maxRatio)
        scale = (minScale,maxScale)
        if (scale[0] > scale[1]) or (ratio[0] > ratio[1]):
            raise ValueError("RandomErasing range should be of kind (min, max)")
        if scale[0] < 0 or scale[1] > 1:
            raise ValueError("RandomErasing range of scale should be between 0 and 1")
        if p < 0 or p > 1:
            raise ValueError("RandomErasing range of random erasing probability should be between 0 and 1")
